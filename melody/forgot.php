...
if(isset($_POST['forgotpass'])) {
    ...
    if($errMsg == '') {
        try{
            $stmt = $connect->prepare('SELECT contrasena, secretpin FROM usuario WHERE secretpin = :secretpin');
            $stmt->execute(array(
                ':secretpin' => $secretpin 
            ));
            $data = $stmt-> fetch(PDO::FETCH_ASSOC);
            if($secretpin == $data['secretpin']){
                $viewpass = 'su contraseña es: '. $data['contrasena'] . '<br> <a href="login.php">  Ingrese ahora!! </a>';
